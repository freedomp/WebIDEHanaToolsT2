...
sap.ui.define([
	...
	var Utils = {
		...
		_getComponentName: function(oComponent) {
			...
			if (oComponent) {
				var oComp;
				if (oComponent.getAppComponent) {
					// in case of smart templates the actual application component needs to be retrieved via getAppComponent
					oComp = oComponent.getAppComponent();
				} else {
					oComp = oComponent;
				}
				if (oComp) {
					sComponentName = oComp.getMetadata().getName();
				}
			}
