...
sap.ui.define(['jquery.sap.global',
	...
	function($, Opa, OpaPlugin, PageObjectFactory, Utils, Ui5Object, Matcher, AggregationFilled, PropertyStrictEquals) {
		 ...
		 * <li> matchers - a single matcher or an array of matchers @see sap.ui.test.matchers. All of the matchers have to match. Check will not be called if the matchers filtered out all controls before. Matchers can change the value, they are matching and pass the new value as a result.</li>
		...
		fnOpa5.prototype.waitFor = function (oOptions) {
			...
			oOptions.check = function () {
				...
				if (aMatchers && aMatchers.length) {
					...
					var aMatchedValues = [];
					aResult.forEach(function (oControl) {
				...
				if (fnOriginalCheck) {
					return this._executeCheck(fnOriginalCheck, aResult);
				}
			...
			if (fnOriginalSuccess) {
				oOptions.success = function () {
					fnOriginalSuccess.call(this, aResult);
				};
			}
		 ...
		 * @param {function} [mPageObjects.<your page object name>.namespace] Namespace prefix for the page object's actions and assertions, default: sap.ui.test.opa.pageObject.
