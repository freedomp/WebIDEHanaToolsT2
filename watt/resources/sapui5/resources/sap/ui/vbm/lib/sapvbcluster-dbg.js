...
VBI.Clustering = function(target) {
	...
	VBI.Clustering.Cluster = function() {
		...
		cluster.recCheck = function(nodes, aNode1, eix1, eix1s, aNode2, eix2, eix2s, aboveLod, vo, edgeLod, log2ComplX, distThreshold, bMergeMode, lvl) {
			var listBOs = [];
			var inserts = [
				0, 0
			];
			...
			var res = cluster.analyzePath(listBOs, aNode1, eix1, eNode2, aboveLod, edgeLod, log2ComplX, distThreshold, bMergeMode);
			...
			if (res.nLod == res.oLod) {
				if (cluster.NodeMerge(res.nLod, aNode2, eix2, aNode1, res.i) && eix2) {
					if (eix2) {
						inserts = cluster.recCheck(nodes, aNode1, res.i - 1, res.i - 1, aNode2, eix2 - 1, eix2 - 1, res.nLod, vo, edgeLod, log2ComplX, distThreshold, bMergeMode, lvl + 1);
					}
				}
				...
				inserts = cluster.recCheck(nodes, aNode2, eix2, eix2s, aNode1, res.i, eix1s, eNode2.lod - 1, vo, edgeLod, log2ComplX, distThreshold, bMergeMode, lvl + 1);
				if (listBOs.length > 1) {
...
// continue; // do not connect sub levels over outer edges
				...
				var aboveLod = -1000, bMergeMode = 0;
				...
				if (bSwitchNodes) {
					cluster.recCheck(nodes, aNode2, eix2, eix2, aNode1, eix1, eix1, aboveLod, vo, edgeLod, log2ComplX, distThreshold, bMergeMode, 0);
				} else {
					cluster.recCheck(nodes, aNode1, eix1, eix1, aNode2, eix2, eix2, aboveLod, vo, edgeLod, log2ComplX, distThreshold, bMergeMode, 0);
				}
...
// };
		...
		cluster.r = function(x) {
			return Math.round(1000 * x) / 1000;
		};

		cluster.analyzePath = function(BOs, nodeChain, index, eNode2, aboveLod, edgeLod, log2ComplX, distThreshold, altLevel) {
			...
			BOs.push(newBO);
			if ((index == 0) || (myLod < eNode1.lod) || altLevel) {
				return {
					i: index,
					nLod: myLod,
					oLod: eNode1.lod
			...
			return cluster.analyzePath(BOs, nodeChain, index - 1, eNode2, myLod, edgeLod, log2ComplX, distThreshold);
		};
		...
		cluster.fillClusterConfig = function(myResult, baseConfig, minLOD, bAnimationAllowed) {
			...
			elte.anim = (VBI.m_bIsMobile || !bAnimationAllowed) ? undefined : cluster.m_animated;
