sap.ui.jsfragment("sap.watt.ideplatform.plugin.commonrunners.view.RunConfigUi", {
	_oController: null,
	...
	createContent: function(oController) {
		...
		// -------- Generic extensions --------
		this._addGenericExtensions(oController, oRunConfigControl);
	...
	_addParamLineTemplate: function(oController) {
		...
		var oDeleteParam = new sap.ui.commons.Button({
			...
			press: that._oController.deleteParameterLine,
			layoutData: new sap.ui.layout.GridData({
				span: "L1 M1 S1"
			})
		});
	...
	_addGenericExtensions: function(oController, oRunConfigControl) {
		// treat all configuration elements starting with "oControls" as UI5 controls,
		// wire them up with our UI in the order they are returned
		Object.keys(oController._oConfigurationUiElements).filter(function(sConfigKey) {
			return sConfigKey.indexOf("oControls") === 0;
		}).map(function(sConfigKey) {
			var oControls = oController._oConfigurationUiElements[sConfigKey];
			// results can either be a single control or an array of controls
			if (Array.isArray(oControls)) {
				return oControls;
			} else if (typeof oControls === "object") {
				return [oControls];
			}
			return [];
		}).forEach(function(aControls) {
			aControls.forEach(function(oControl) {
				oRunConfigControl.addContent(oControl);
			});
		});
	}
