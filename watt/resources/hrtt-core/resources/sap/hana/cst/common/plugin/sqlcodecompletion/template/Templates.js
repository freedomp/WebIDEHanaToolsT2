...
define([], function() { //$NON-NLS-0$
    ...
    function Template(name, prefix, description, template, category) {
        this.name = name;
        this.prefix = prefix;
        this.description = description;
        this.template = template;
        // @CHANGE
        this.category = category || "sql.template";
        this._parse();
    }
                ...
                if (variable !== undefined) {
                    switch (segment) {
                        ...
                        default:
                            ...
                            if (g.data && g.data.values) {
                                segment = g.data.values[0];
                            }
                            g.positions.push({
    ...
    TemplateContentAssist.prototype = /** @lends orion.editor.TemplateContentAssist.prototype */ {
        ...
        addTemplates: function(json) {
            ...
            for (var j = 0; j < json.length; j++) {
                var oTemplate = json[j];
                var aPrefixes = oTemplate.prefix.split(",");
                for (var k = 0; k < aPrefixes.length; k++) {
                    templates.push(new Template(oTemplate.name, aPrefixes[k], oTemplate.description, oTemplate.template, oTemplate.category));
                }
        ...
        // @CHANGE
        computeKeywordProposalsWithPrefixChar: function(prefixChar, prefix, context) {
            var proposals = [];
            return proposals.concat(this.getKeywordProposals(prefix, context, prefixChar));
        },
        ...
        // @CHANGE
        getKeywordProposals: function(prefix, context, prefixChar) {
            var proposals = [];
            ...
            if (keywords) {
                ...
                for (var i = 0; i < keywords.length; i++) {
                    var oKeyword = keywords[i];
                    var sKeyword = "";
                    var sCategory = "sql.keyword";
                    if (typeof oKeyword === "string") {
                        sKeyword = oKeyword;
                        sCategory = "sql.keyword";
                    } else if (typeof oKeyword === "object") {
                        sKeyword = oKeyword.text;
                        sCategory = oKeyword.category;
                    }
                    
                    var sDescription = sKeyword;
                    var sProposal = sKeyword;
                    
                    if (prefixChar) {
                        // the prefix contains the indicated prefix char, so include it in proposal
                        var iIdx = prefix.indexOf(prefixChar);
                        if (iIdx === 0) {
                            sProposal = prefixChar + sProposal;
                            sKeyword = prefixChar + sKeyword;
                        }
                    }
                    
                    var isMatched = false;
                    if (caseSensitive) {
                        isMatched = (sKeyword.indexOf(prefix) === 0);
                    } else {
                        // all the existing keywords are in the lower case
                        isMatched = (sKeyword.toLowerCase().indexOf(prefix.toLowerCase()) === 0);
                    }
                    if (isMatched) {
                        var proposal = {
                            // @CHANGE
        ...
        getTemplateProposals: function(prefix, offset, context) {
            ...
            for (var t = 0; t < templates.length; t++) {
                ...
                if (template.match(prefix, context.caseSensitive)) {
                    ...
                    // @CHANGE
                    // check if this proposal already be populated
                    var oLastProposal = proposals[proposals.length - 1];
                    if (oLastProposal && proposal) {
                        if (oLastProposal.name === proposal.name) {
                            continue;
                        }
                    }
