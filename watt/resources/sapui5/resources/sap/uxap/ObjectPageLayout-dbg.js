...
sap.ui.define([
		...
		'./library'
	], function(jQuery, ResizeHandler, Control, CustomData, ScrollEnablement, ObjectPageSubSection, ObjectPageSubSectionLayout, LazyLoading, ITBHelper, ABHelper, library) {
	"use strict";
	 ...
	 */
	var ObjectPageLayout = Control.extend("sap.uxap.ObjectPageLayout", /** @lends sap.uxap.ObjectPageLayout.prototype */ { metadata : {
		...
		properties : {
			 ...
			 */
			subSectionLayout : {type : "sap.uxap.ObjectPageSubSectionLayout", defaultValue : ObjectPageSubSectionLayout.TitleOnTop},
	...
	ObjectPageLayout.prototype._onScroll = function (oEvent) {
		...
		if ((oHeader && this.getShowHeaderContent()) || this.getShowAnchorBar()) {
	...
	ObjectPageLayout.prototype._restoreFocusAfter = function (fnMoveNavBar) {
		...
		if (sap.ui.Device.system.phone !== true) { // FIX - can not convert to expanded on windows phone
	...
	ObjectPageLayout.prototype._getHeaderDesign = function () {
		var oHeader = this.getHeaderTitle(),
			sDesign = sap.uxap.ObjectPageHeaderDesign.Light;
	...
	ObjectPageLayout.prototype._getHeaderContent = function () {
		...
		if (!this.getAggregation("_headerContent")) {
			this.setAggregation("_headerContent", new sap.uxap.ObjectPageHeaderContent({
	 ...
	 * @static
	 * @param oControl - the control to find ObjectPageLayout for
	 * @private
	 * @returns {*}
	 */
	ObjectPageLayout._getClosestOPL = function(oControl) {
