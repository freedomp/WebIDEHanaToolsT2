...
sap.ui.base.Object.extend("sap.collaboration.components.socialtimeline.datahandlers.TimelineDataHandler",{
	...
	setBusinessObject: function( oObject ){
		...
		this._sBusinessObjectKey = oObject.key;
		this._sBusinessObjectName = oObject.name;
	...
	/**
	 * Get external object from the internal object
	 ...
	 * @memberOf sap.collaboration.components.socialtimeline.datahandlers.TimelineDataHandler
	 */
	getExternalObjectFromInternalObject: function(){
		var that = this;
		var oInternalBO = { 
		...
		var oPromise = 
			this._oSMIntegrationDataHandler.mapInternalBOToExternalBO(oInternalBO)	// 1 Get external object mapping
			.then(function(oExternalBOMapping){
				oExternalBOMapping.Name = that._sBusinessObjectName;
				return that._oJamDataHandler.getExternalObject(oExternalBOMapping);	// 2- Get external object 
			});
	...
	getTimelineData: function(oFilter){
		...
		else if(oFilter.type === this._oFilterConstants.FILTER_TYPE.systemUpdates
				|| oFilter.type === this._oFilterConstants.FILTER_TYPE.custom){
	...
	_getExternalObjectFeedEntries: function(){
		...
		var oPromise = 
			this.getExternalObjectFromInternalObject()
			.then(function(oExternalBO){
				that.setCurrentExternalBOId(oExternalBO.Id);
				...
				// if the next link in buffer is defined, then get the next page of feed entries defined by the next link
					...
					return that._oJamDataHandler.getFeedEntries(oExternalBO, that._sFeedEntriesNextLink); 
				}
			})
	...
	_getTimelineEntries: function(oFilter){
		...
		var gettingTimelineEntries = 
			...
			.then(function(aTimelineEntries){
	...
	/**
	 * Setter for the current External Id of the BO
	 * 
	 * @params {string} sExternalBOId - The External Id of the BO
	 * @memberOf sap.collaboration.components.socialtimeline.datahandlers.TimelineDataHandler
	 */
	setCurrentExternalBOId: function( sExternalBOId ){
		this._sCurrentExternalBOId = sExternalBOId;
	},
	/**
	 * Getter for the current External Id of the BO
	 * 
	 * @returns {string} External Id of the current BO 
	 * @memberOf sap.collaboration.components.socialtimeline.datahandlers.TimelineDataHandler
	 */
	getCurrentExternalBOId: function(){
		return this._sCurrentExternalBOId;
	}
});
