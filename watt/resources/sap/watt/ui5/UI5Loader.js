define({
	...
	load: function () {
		...
		var bUI5FromHelium = !(sap.watt.getEnv("ui5_from_cdn") === true);
		...
		var fnLoadScriptSync = function () {
			...
			if (aScriptSegments) {
				...
				if (sap.watt.getEnv("staticWSMode")) {
					sScriptUrl = sap.watt.getEnv("staticWSUI5Root") + "sap-ui-core"+ sDbgSuffix + ".js";
				}
				sap.watt.includeScript(sScriptUrl).then(function() {
					var ui5Initialized = Q.defer();
					sap.ui.getCore().attachInit(function() {
    					ui5Initialized.resolve();
					});
					return ui5Initialized.promise;
				}).then(fnLoadScriptSync);
			} else {
		...
		if (/^(java|hcproxy|local_hcproxy)$/gi.test(sap.watt.getEnv("server_type")) && bUI5FromHelium) {
