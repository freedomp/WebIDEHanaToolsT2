...
VBI.addSceneLabelFunctions = function(scene) {
	...
	scene.InternalRenderLabelsOld = function(canvas, dc) {
		...
		// separate both to not switch fonts for each label ( performance reason )
		var aVO = scene.m_VOS;
		var nI, len, aLabels, label, nY;
		for (nI = 0, len = aVO.length; nI < len; ++nI) {
			aLabels = aVO[nI].getLabelData(true);
			for (nY = 0; nY < aLabels.length; ++nY) {
				label = aLabels[nY];
				label.SetDimensions(dc);
				label.AlignLabel();
				var textcolor = label.GetLabelTextColor();
				var substrings = label.m_Text.split(/\r\n/);
				scene.InternalDrawLabelTexts(dc, label, textcolor, substrings);
			}
		}

		VBI.Utilities.SetTextAttributes(dc, "12px SAP-icons", undefined, undefined, "center", "middle");
		for (nI = 0, len = aVO.length; nI < len; ++nI) {
			aLabels = aVO[nI].getLabelData(false);
			for (nY = 0; nY < aLabels.length; ++nY) {
				label = aLabels[nY];

				scene.InternalDrawLabelIcons(dc, label);
			}
		}
	};
