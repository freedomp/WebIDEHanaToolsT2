...
define(function() { //$NON-NLS-0$
	...
	function Template (name, prefix, description, template, helpDescription) {
		this.name = name;
		this.prefix = prefix;
	...
	Template.prototype = /** @lends orion.editor.Template.prototype */ {
		...
		match: function(ns, prefix) {
			var ret;
			if (ns!="") {
				ret = this.name.toLowerCase().indexOf(ns.toLowerCase()+"."+prefix.toLowerCase()) === 0;
			} else {
				ret = this.prefix.toLowerCase().indexOf(prefix.toLowerCase()) === 0;
			}
			return ret;
		},
		_parse: function() {
			...
			template.replace(reg, function(group, text1, index) {
	...
	TemplateContentAssist.prototype = /** @lends orion.editor.TemplateContentAssist.prototype */ {
		addTemplates: function(json) {
			...
			for (var j = 0; j < json.length; j++) {
				templates.push(new Template(json[j].name, json[j].prefix, json[j].description, json[j].template, json[j].helpDescription));
			}
		},
		...
		computeProposals: function(buffer, offset, context) {
			...
			proposals = proposals.concat(this.getTemplateProposals(context.ns, prefix, offset, context));
		...
		getTemplateProposals: function(ns, prefix, offset, context) {
			var proposals = [];
			var templates = this.getTemplates();
			for (var t = 0; t < templates.length; t++) {
				var template = templates[t];
				if (template.match(ns, prefix)) {
