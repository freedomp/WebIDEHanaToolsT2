...
sap.ui.define([
	...
	'./HierarchicalSelect',
	'./library'
], function (PlacementType, Popover, Toolbar, Item, ResizeHandler, ScrollEnablement, HierarchicalSelect, library) {
	"use strict";
	...
	AnchorBar.prototype.init = function () {
		...
		this._bHasButtonsBar = sap.ui.Device.system.tablet || sap.ui.Device.system.desktop;
		...
		if (this._bHasButtonsBar) {
			...
			this._oArrowLeft = this._getArrowLeft(this._oArrowLeft, this);
			this._oArrowRight = this._getArrowRight(this._oArrowRight, this);
	...
	AnchorBar.prototype.getSelectedSection = function () {
		...
		if (oSelectedButton
			&& (oSelectedButton instanceof sap.m.Button)
	...
	AnchorBar.prototype._requestScrollToSection = function (sRequestedSectionId) {
		...
		if (this.getParent() instanceof sap.uxap.ObjectPageLayout) {
			...
			if (oRequestedSection instanceof sap.uxap.ObjectPageSubSection &&
				oRequestedSectionParent instanceof sap.uxap.ObjectPageSection) {
	...
	AnchorBar.prototype._isMobileScenario = function () {
		...
		if (sap.ui.Device.media.hasRangeSet(sap.ui.Device.media.RANGESETS.SAP_STANDARD_EXTENDED)) {
			...
			if (oRange && oRange === 'Phone') {
				return true;
			}
		}

		return jQuery("html").hasClass("sapUiMedia-Std-Phone");
	};
	...
	AnchorBar.prototype._getArrowLeft = function (oArrowControl, oParent) {

		return sap.m.ImageHelper.getImageControl(this.getId() + "-arrowScrollLeft", oArrowControl, oParent, {
			src: "sap-icon://navigation-left-arrow"
		}, ["sapMITBArrowScroll", "sapMITBArrowScrollLeft", "sapUxAPAnchorBarArrowScroll", "sapUxAPAnchorBarButton"]); //same classes as the iconTabBar as requested by Ux

	};

	AnchorBar.prototype._getArrowRight = function (oArrowControl, oParent) {

		return sap.m.ImageHelper.getImageControl(this.getId() + "-arrowScrollRight", oArrowControl, oParent, {
			src: "sap-icon://navigation-right-arrow"
		}, ["sapMITBArrowScroll", "sapMITBArrowScrollRight", "sapUxAPAnchorBarArrowScroll", "sapUxAPAnchorBarButton"]); //same classes as the iconTabBar as requested by Ux
	...
	AnchorBar.prototype._adjustSize = function () {
		...
		var sNewMode = this._isMobileScenario() ?
	 ...
	 * @private
	 */
	AnchorBar.prototype.ontap = function (oEvent) {
	...
	AnchorBar.prototype.onAfterRendering = function () {
		...
		if (this._bHasButtonsBar) {
			...
			jQuery.sap.delayedCall(AnchorBar.DOM_CALC_DELAY, this, this._adjustSize);

			var $ArrowLeft = this._oArrowLeft.$();
			var $ArrowRight = this._oArrowRight.$();
	...
	AnchorBar.prototype._computeBarSectionsInfo = function () {
		...
		//chrome, safari
		if (this._bRtlScenario && sap.ui.Device.browser.webkit) { // Reverse all positions as the scroll 0 is at the far end (first item = maxPosition, last item = 0)
