...
module.exports = function (driver, By, until, configuration) {
	...
	var mappings = {
		...
		nextButton_d : {type : 'css' , path :  "button[type='button'][title='Next'][aria-disabled=true]"},
		...
		templateSelectionTitle  : {type : 'css' , path : "span[title='1 Template Selection']"},
	...
	return {
		...
		next : function() {
			driver.wait(until.elementLocated(utils.toLocator(mappings.nextButton)), configuration.defaultTimeout);
			return driver.myWaitAndClick(utils.toLocator(mappings.nextButton), configuration.defaultTimeout).thenCatch(function (oError) {
		...
		enterProjectName : function(projectName) {
			...
			driver.wait(until.elementLocated(projectNameTextbox), configuration.defaultTimeout);
			return driver.findElement(utils.toLocator(mappings.projectNameTextbox)).then(function() {
				return driver.myWaitAndClick(utils.toLocator(mappings.projectNameTextbox), configuration.defaultTimeout).then(function() {
				return driver.myWaitAndSendKeys(projectName, utils.toLocator(mappings.projectNameTextbox), configuration.defaultTimeout).thenCatch(function (oError) {
					return driver.myWaitAndSendKeys(projectName, utils.toLocator(mappings.projectNameTextbox), configuration.defaultTimeout);
				});
				});
			});
		},
		...
		configureTemplateCustomizationstep : function(){
			...
			return driver.myWaitAndSendKeys("DamageReportSet",templateCustomizationODataCollection, configuration.defaultTimeout).then(function(){
				...
				that.enterTemplateCustomizationText("ClaimedsumCurrencyCode", 1, 'Units attribute from the OData collection');
			}).then(function(){
		...
		generateNewMDWithPhotosProject : function(){
			...
			return this.openFromWelcomePerspective().then(function(){
				...
				return that.enterProjectName(configuration.projectName);
			}).then(function(){
				console.log("next");
				return that.next();
			}).then(function(){
