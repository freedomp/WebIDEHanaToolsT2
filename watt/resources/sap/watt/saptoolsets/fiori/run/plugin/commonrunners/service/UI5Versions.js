	...
	return {
		...
		getUI5Versions: function() {
			...
			if (sap.watt.getEnv("internal")) {
				...
				return that._getUI5Versions(oUi5Version, oUi5VersionInternal);
			} else {
				return that._getUI5Versions(oUi5Version);
			}
		},
				...
				if (oNeoApp !== null) {
					if (oNeoApp.routes !== null && oNeoApp.routes !== undefined) {
						for (var i = 0; i < oNeoApp.routes.length; i++) {
							if ((oNeoApp.routes[i].target.version) && (oNeoApp.routes[i].target.type === "service") && (oNeoApp.routes[i].target.name ===
								"sapui5")) {
		...
		_getUI5Versions: function(oUi5Version, oUi5VersionInternal) {
		...
		sortOutVersionsBelowAppDescriptorVersion: function(sUI5MinimalVersion, aUi5Version) {
		...
		getUI5VersionFromAppDescriptor: function(oDocument) {
			var that = this;
