...
define([], function() {
	...
	return {
		 ...
		 * @return {Boolean}
		 */
		looselyMatches: function(prefix, target, caseSensitive) {
			...
			if (caseSensitive) {
				return false;// stop to compare here for case sensitive filter
			}
	
			var lowerCase = target.toLowerCase();
			if (this.startsWith(lowerCase, prefix.toLowerCase())) {
				return true;
			}
