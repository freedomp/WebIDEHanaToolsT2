define(["STF", "sinon"], function(STF, sinon) {
	...
	var suiteName = "js_codecompletion_service_tests";
	var suiteWindowObj;
	var sandbox;
		...
		"configures": {
			"services": {
				"jscodecompletion:libraries": [{
					...
					"name": "sapui5",
					"version": "1.29.1",
					"libIndexFile": "sap.watt.toolsets.javascript/service/indexFiles/ui5/1.29.1.zip",
					"libTemplate": "sap.watt.toolsets.javascript/service/template/ui5/1.29.1.zip",
	...
	describe("JS Code Completion Service", function() {
		before(function() {
...
///////////////////////////////////////////
		...
		it("Check [getWordSuggestions] for duplicated proposals case1(JIRA: WATTSIN-153)", (function() {
			...
			return getProposals(oContentStatus).then(function(result) {
				...
				expect(!checkProposal(proposals, "proposal", "namespaceabc.cba.Def()"), "return expected proposal").to.be.true;
			});
		}));
		...
		it("Check [getWordSuggestions] for snippet: 'new' keyword snippet", (function() {
			...
			return getProposals(oContentStatus).then(function(result) {
				...
				expect(proposals).to.exist;  				
				expect(proposals, "there are " + proposals.length + " proposals").to.have.length.above(0);
				expect(checkProposal(proposals, "proposal", "w"), "return expected proposal").to.be.true;
			});
		}));
