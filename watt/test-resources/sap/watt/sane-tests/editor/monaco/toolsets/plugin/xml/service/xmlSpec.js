define(["STF"], function(STF) {
	...
	var suiteName = "xml_service_tests";
	var suiteWindowObj;
	...
	var sCoCoContent =
		'<core:View controllerName="proj.view.Detail" xmlns:core="sap.ui.core" xmlns:mvc="sap.ui.core.mvc" xmlns="sap.m"><Page id="detailPage"><content><core:ScrollBar ><mvc:TemplateView ><Button > </mvc:TemplateView></core:ScrollBar></content></Page></core:View>';
	var sCoCoContent2 =
		'<core:View controllerName="proj.view.Detail" xmlns:core="sap.ui.core" xmlns="sap.m" xmlns:comm="sap.ui.commons"><Page id="detailPage">Button</content></Page></core:View>';
	...
	describe("Xml Code Completion Service", function() {
		before(function() {
		...
		//----------------------------------------------------------------------------------------	
		//New test for the actual content of the proposals
		it("offset for tag with namespace proposals", (function() {
			var i = 0;
			var iOffset = sCoCoContent.indexOf("><But") + 4;
			var oContentStatus = {
				buffer: sCoCoContent,
				offset: iOffset
			};
			return oXMLCodeCompletionService.getWordSuggestions(oContentStatus).then(function(result) {
				expect(result.proposals, "Proposals exist").to.have.length.above(0);
				for (i = 0; i < result.proposals.length; i++) {
					if (result.proposals[i].description === "Button in sap.m" && result.proposals[i].categoryDesc === "SNIPPET") {
						expect(result.proposals[i].overwrite, "overwrite true").to.be.true;
						expect(result.proposals[i].proposal.substring(0, 6), "proposal true").to.equal("Button");
					}
					if (result.proposals[i].description === "Button in sap.m" && !result.proposals[i].categoryDesc) {
						expect(result.proposals[i].overwrite, "overwrite true").to.be.true;
						expect(result.proposals[i].proposal.substring(0, 6), "proposal true").to.equal("Button");
					}
				}
				expect(result.prefix, "the prefix is empty").to.equal("Bu");
			});
		}));

		//New test for the actual content of the proposals
		it("offset for tag with namespace proposals2", (function() {
			var i = 0;
			var iOffset = sCoCoContent2.indexOf("Button") + 4;
			var oContentStatus = {
				buffer: sCoCoContent2,
				offset: iOffset,
				prefix: "Butt"
			};
			return oXMLCodeCompletionService.getWordSuggestions(oContentStatus).then(function(result) {
				...
				for (i = 0; i < result.proposals.length; i++) {
					if (result.proposals[i].description === "Button in sap.m" && result.proposals[i].categoryDesc === "SNIPPET") {
						expect(result.proposals[i].overwrite, "overwrite true").to.be.true;
						expect(result.proposals[i].proposal.substring(0, 7), "proposal true").to.equal("<Button");
					}
					if (result.proposals[i].description === "Button in sap.m" && !result.proposals[i].categoryDesc) {
						expect(result.proposals[i].overwrite, "overwrite true").to.be.true;
						expect(result.proposals[i].proposal.substring(0, 7), "proposal true").to.equal("<Button");
					}
					if (result.proposals[i].description === "Button in sap.ui.commons" && result.proposals[i].categoryDesc === "SNIPPET") {
						expect(result.proposals[i].overwrite, "overwrite true").to.be.true;
						expect(result.proposals[i].proposal.substring(0, 12), "proposal true").to.equal("<comm:Button");
						expect(result.proposals[i].proposal.substring(result.proposals[i].proposal.length - 14, result.proposals[i].proposal.length), "proposal true").to.equal("</comm:Button>");
					}
					if (result.proposals[i].description === "Button in sap.ui.commons" && !result.proposals[i].categoryDesc) {
						expect(result.proposals[i].overwrite, "overwrite true").to.be.true;
						expect(result.proposals[i].proposal.substring(0, 12), "proposal true").to.equal("<comm:Button");
					}
				}
				expect(result.prefix,  "the prefix is empty ").to.equal("Butt");
			});
		}));
		...
		it("offset for tag with namespace proposals", (function() {
			...
			return oXMLCodeCompletionService.getWordSuggestions(oContentStatus).then(function(result) {
				for (i = 0; i < result.proposals.length; i++) {
					if (result.proposals[i].description === "ScrollBar in sap.ui.core") {
						expect(result.proposals[i].overwrite, "overwrite true").to.be.true;
						expect(result.proposals[i].proposal, "proposal true").to.equal("core:ScrollBar");
						break;
					}
				}
				expect(result.prefix, "the prefix is empty ").to.equal("");
		...
		it("offset for tag with namespace proposals - attributes", (function() {
			var i = 0;
			var iOffset = 211;
			var oContentStatus = {
				buffer: sCoCoContent,
			...
			return oXMLCodeCompletionService.getWordSuggestions(oContentStatus).then(function(result) {
				for (i = 0; i < result.proposals.length; i++) {
					//Attribute
					if (result.proposals[i].description === "scrollPosition") {
						expect(result.proposals[i].overwrite, "overwrite true").to.be.true;
						expect(result.proposals[i].proposal, "Attribute proposal true").to.equal("scrollPosition");
					}
					//inherited
					if (result.proposals[i].description === "busy") {
						expect(result.proposals[i].overwrite, "overwrite true").to.be.true;
						expect(result.proposals[i].proposal, "Attribute proposal true").to.equal("busy");
					}
					//event
					if (result.proposals[i].description === "scroll") {
						expect(result.proposals[i].overwrite, "overwrite true").to.be.true;
						expect(result.proposals[i].proposal, "Attribute proposal true").to.equal("scroll");
					}
				}
				expect(result.prefix, "the prefix is empty").to.equal("");
			});
