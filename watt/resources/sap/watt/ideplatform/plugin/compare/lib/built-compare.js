		...
		if (command.hrefCallback) {
			...
			if (useImage) {
				if (command.hasImage()) {
					...
					element.classList.add("riverControlSmall"); //$NON-NLS-0$
					//element.classList.add("sapUiBtnLite"); //$NON-NLS-0$
					element.classList.add("webIDELayout"); //$NON-NLS-0$
					element.classList.add("sapUiBtn"); //$NON-NLS-0$
					element.classList.add("sapUiBtnNorm"); //$NON-NLS-0$
					element.classList.add("sapUiBtnS"); //$NON-NLS-0$
					element.appendChild(document.createTextNode(command.name));
				}
			} else {
				element.classList.add("commandButton"); //$NON-NLS-0$
...
define('orion/compare/nls/root/messages',{
	"Copy current change from right to left": "Copy from right to left",
	"Copy current change from left to right": "Copy from left to right",
	...
	"Next diff block": "Next",
	"Previous diff block": "Previous",
	"Next diff change": "Next",
	"Previous diff change": "Previous",
		...
		if (lineIndex === undefined) {
			...
			if(lineIndex >= 0 ){
				...
				if(annotationIndex === this._diffNavigator.getCurrentBlockIndex()){
					style.backgroundColor = conflict ? "red" :"#007cc0"; //$NON-NLS-1$ //$NON-NLS-0$
				}
        ...
        copyTo: function(left){
			...
			if(currentDiff){
				var textToCopy = left ? this._rightTextView.getText(currentDiff.oldA.start , currentDiff.oldA.end) : this._leftTextView.getText(currentDiff.newA.start , currentDiff.newA.end);
    				...
    				this._diffNavigator.iterator.cursor().newA.end = this._diffNavigator.iterator.cursor().newA.start;
			}
		},
	...
	TextView.prototype = /** @lends orion.editor.TextView.prototype */ {
		...
		*/
		...
		_onModelChanged: function(modelChangedEvent) {
			...
			jQuery(this._parent).trigger("compareEditorModify");
...
exports.CompareCommandFactory = (function() {
	...
	CompareCommandFactory.prototype = {
		...
		initCommands: function(compareWidget){	
			...
			commandService.registerCommandContribution(commandSpanId, "orion.compare.copyToLeft", 114, null, false, new mKeyBinding.KeyBinding(37/*left arrow key*/, true, false, true)); //$NON-NLS-0$
			commandService.registerCommandContribution(commandSpanId, "orion.compare.copyToRight", 115, null, false, new mKeyBinding.KeyBinding(39/*left arrow key*/, true, false, true)); //$NON-NLS-0$
			commandService.registerCommandContribution(commandSpanId, "orion.compare.nextDiff", 111, null, false, new mKeyBinding.KeyBinding(40/*down arrow key*/, true)); //$NON-NLS-0$
			commandService.registerCommandContribution(commandSpanId, "orion.compare.prevDiff", 110, null, false, new mKeyBinding.KeyBinding(38/*up arrow key*/, true)); //$NON-NLS-0$
			if(compareWidget.options.wordLevelNav){
				commandService.registerCommandContribution(commandSpanId, "orion.compare.nextChange", 113, null, false, new mKeyBinding.KeyBinding(40/*down arrow key*/, true, true)); //$NON-NLS-0$
				commandService.registerCommandContribution(commandSpanId, "orion.compare.prevChange", 112, null, false, new mKeyBinding.KeyBinding(38/*up arrow key*/, true, true)); //$NON-NLS-0$
			} else {
