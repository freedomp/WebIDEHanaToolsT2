...
VBI.CalloutWindow = function() {
	...
	callout.CalcDivPosition = function() {
		...
		if (pos.length > 5) {
			...
			aOffsets = hs.GetInstanceOffsets([
				tmppos[0], tmppos[1], tmppos[0], tmppos[1]
			]);
			for (nJ = 0, len = aOffsets.length; nJ < len; ++nJ) {
	...
	callout.Create = function(target) {
		...
		if (callout.m_refParent && !callout.m_oCallout) {
			...
			if (hs) {
				...
				if (hs.m_bThumbnailedCanvases) { // create it hidden in case we are in thumbnail mode
					callout.m_oCallout.m_Div.m_VBIHidden = true;
					callout.m_oCallout.m_Div.style.display = 'none';
				}
				// call hook.....................................................//
...
// helper functions.......................................................//
	...
	legend.Create = function(target) {
		if (legend.m_bRenew) {
			...
			if (hs) {
				...
				// try to show it................................................//
				legend.m_oLegend.m_Div = hs.m_Div.appendChild(legend.m_oLegend.m_Div);
	...
	legend.adjustPosition = function() {
		var scHeight = legend.GetHostingScene().m_Div.clientHeight;
		var lgHeight = legend.m_oLegend.m_Div.clientHeight;
		var lgTop = parseInt(legend.m_oLegend.m_Div.style.top, 10);
		var diff = scHeight - (lgTop + lgHeight);
		if (diff < 0) {
			jQuery(legend.m_oLegend.m_Div).css('top', lgTop + diff + 'px');
		}
...
// var row = e.srcElement.parentElement.id;
// var test = legend.getId( '', 'content-tablerow' );
		// __item0-__legend0-0 ???
		var sourceElement = e.target || e.srcElement;
		var rowNumber = (sourceElement.parentElement.id).slice((legend.getId('', 'content-tablerow')).length);
		// console.log("content clicked; row = " + rowNumber);
