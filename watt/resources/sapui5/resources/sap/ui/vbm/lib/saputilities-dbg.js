...
VBI.Utilities.CreateContainer = function(id, key, left, top, width, height, tooltip, zIndex, bOmitClass) {
	"use strict";
...
// return [
// 50, (style.paddingTop ? parseInt(style.paddingTop, 10) : 0) + (style.paddingBottom ? parseInt(style.paddingBottom, 10) : 0) + (style.height ?
// parseInt(style.height, 10) : 0) + (style.borderTop ? parseInt(style.borderTop, 10) : 0) + (style.borderBottom ? parseInt(style.borderBottom, 10) :
// 0) + +40
// ];
// }
	};
...
VBI.Utilities.GetClippedPolygon = function(pointarray, Xoffset, rcviewport) {
	...
	for (var nJ = 0; nJ <= 3; ++nJ, offset = 0) {
		...
		for (var nK = 0; nK <= inputlist.length - 2; nK += 2) {
			...
			if (bEInside) {
				if (!bSInside) {
					intersection = VBI.Utilities.LineLineIntersection(S, E, clipEdge[0], clipEdge[1]);
					if (intersection) {
