...
define(["./TextInputHelper",
    ...
    function(TextInputHelper, HotKey) {
        ...
        var COMMON_IMAGES_PATH = "/sap/hana/cst/common/images/";
        ...
        SearchDialog.prototype = {
            ...
            _createUI: function() {
                ...
                var oLayoutSearch = new sap.ui.commons.layout.MatrixLayout({
                    column: 2,
                    layoutFixed: false,
                    width: "100%",
                    widths: ["", "40px"]
                });
                var btnRefresh = new sap.ui.commons.Button({
                    text: "",
                    tooltip: "Refresh",
                    icon: "/sap/hana/cst/common/images/refresh.gif",
                    press: function(oEvent) {
                        that._aItems = null;
                        var sInputVal = that.oSearch.getValue();
                        that.updateList(sInputVal);
                    }
                });
                oLayoutSearch.createRow(cell(this.oSearch, {
                    padding: sap.ui.commons.layout.Padding.End
                }), cell(btnRefresh));
                // fnCreateRow(this.oSearch, oMatrix);
                fnCreateRow(oLayoutSearch, oMatrix);
            ...
            updateList: function(sInputVal) {
                ...
                if (sFindValue) {
                    if (that._aItems === null) {
                        this._updateList(sFindValue).then(function(rs) {
                            if (!rs) {
                                rs = [];
                            ...
                            that._aItems = rs;
                            aItems = that._updateListForFiltering(sFindValue);
                            that._displayResult(sInputVal, aItems);
                        }).done();
                    } else {
                        aItems = that._updateListForFiltering(sFindValue);
                        that._displayResult(sInputVal, aItems);
            ...
            _updateList: function(sFindValue) {
                ...
                return this.context.service.xs2CtrlDAO.getBindingHDIServices(true, sFindValue).then(function(rs) {
                    ...
                    if (rs && rs.services && rs.services.length > 0) {
                        aServices = rs.services;
                    }
                    return aServices;
                });
            },

            _updateListForFiltering: function(sFindValue) {
                var that = this;
                var aItems = [];
                if (sFindValue === "%") {
                    return that._aItems;
                    ...
                    if (that._aItems) {
                        for (var i = 0; i < that._aItems.length; i++) {
                            var oService = that._aItems[i];
                            if (oService.name.toUpperCase().indexOf(sFindValue) !== -1) {
                                aItems.push(oService);
                            }
                        } // end for
                    }
                }
            ...
            _cell: function(oContent, oSettings) {
                ...
                jQuery.extend(oDefaultSettings, oSettings);
                return new sap.ui.commons.layout.MatrixLayoutCell(oDefaultSettings);
            }
