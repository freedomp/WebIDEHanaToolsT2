...
define("rndrt/Utils", ["./StringBuffer"], function(StringBuffer) {
  ...
  var Utils = {objectCreate:function(prototype) {
    ...
    return stringParam
  }, get:function(url, acceptHeader, onBeforeSend, onLoaded) {
    ...
    if(document.defaultView.navigator.appName === "Node.js jsDom") {
      return require("fs").readFileSync(url, {encoding:"utf8"})
    }
    var http = createR();
    var getString = "GET";
    var openString = "open";
    var sendString = "send";
    http[openString](getString, url, true);
    if(acceptHeader != null) {
      http.setRequestHeader("Accept", acceptHeader)
    }
    ...
    while(true) {
      ...
      if(s.charAt(s.length - 1) === ":") {
        ...
        if(s === "rule") {
                ...
                if(s1 === "tc=") {
                  ...
                  if(s1 === "flgc=") {
                    ...
                    if(s1 === "fllwc=") {
                      ...
                      if(s1 === "phrase=") {
                        s1 = this.readNextQuotedWord();
                        phrase = s1.length > 2 ? s1.substring(1, s1.length - 2) : rule;
                        ++line_no
                      }else {
                        if(s1 === "follow=") {
                          break
          ...
          var rule_info = new RuleInfo(phrase, ruleProps, terminal_count, flag_count, rule_flags, role);
...
define("rndrt/Constants", [], function() {
  var Constants = {NUM_UNDEF:-1, NUM_ANYKW:0, NUM_ANYNOTINUSE:1, NUM_EOF:2, NUM_NL:3, NUM_COMMENT1:4, NUM_COMMENT2:5, LAST_NUM:5, ParsingPhase:{TRY_PHASE:0, COMMIT_PHASE:1}};
  return Constants
});
...
define("rndrt/Parser", ["rndrt/Constants", "rndrt/Path", "rndrt/FramePtr", "rndrt/StringWriter", "rndrt/PrintWriter", "rndrt/VMStatus", "rndrt/ErrorState", "rndrt/HookResult", "rndrt/Instruction", "rndrt/Stackframe", "rndrt/ScannerState", "rndrt/Category", "rndrt/Syscodes", "rndrt/RNDContext", "rndrt/ParserTrace", "rndrt/ByteCode", "rndrt/Utils"], function(Constants, Path, FramePtr, StringWriter, PrintWriter, VMStatus, ErrorState, HookResult, Instruction, Stackframe, ScannerState, Category, Syscodes, 
RNDContext, ParserTrace, ByteCode, Utils) {
  ...
  Parser.prototype.vmMatch = function(match_instr) {
    ...
    if(this.match(match_instr.m_la, match_instr.m_is_strict)) {
      var tok = this.lTok();
      if(match_instr.m_is_strict && match_instr.m_la != tok.m_num && tok.m_suspicous_candidate) {
        if(this.m_trace != null) {
          this.m_trace.println("Suspicious candidate may not match weak for a strict match")
        }
        this.setPathFailed();
        return
      }
      ...
      if(match_instr.m_la == this.m_scanner.getActualNUMID() && this.isDirtyMatchOne(this.m_current.getPathLATokenNum())) {
        ...
        if(this.maxSuspiciousMatchesNotExceeded()) {
          ...
          if(this.m_trace != null) {
            this.traceindent();
            this.m_trace.print(" Omitting path (match) because suspicous matches reached maximium. ");
            this.m_trace.print("'" + this.getByteCode().getTokenNameUS(match_instr.m_la) + "'");
            this.m_trace.print("(" + match_instr.m_la + ")");
            this.m_trace.print("have: ");
            this.m_trace.print(this.lTok());
  ...
  Parser.prototype.addAllPathesOfBranch = function(branch_instr) {
  ...
  Parser.prototype.branch = function(first, end, had_a_strict_match) {
    ...
    if(this.m_trace != null) {
      this.traceindent();
      this.m_trace.println("  continue at " + this.m_byte_code.getLref(addr_list.get(branch_to_take_count - 1)))
    }
  ...
  Parser.prototype.vmBranch = function(branch_instr) {
    ...
    var have_anykw = this.m_current.getPathLATokenNum() == Constants.NUM_ANYKW;
  ...
  Parser.prototype.setPathesExhaustedHook = function(hook) {
      ...
      if(this.m_hit_completion_pos) {
        ...
        if(this.m_pathesExhaustedHook != null) {
          ...
          switch(res) {
            ...
            case HookResult.PATHFAILED:
              throw"No PATHFAILED in pathesExhaustedHook";;
            case HookResult.STOP:
              return false;
            default:
              throw"Pathes recovery may not return this result!";
          }
        }
        return this.pathesExhausted()
      }
  ...
  Parser.prototype.pathesExhausted = function() {
    ...
    if(this.m_current.m_la_index > this.getLastSyncPoint()) {
      ...
      if(this.TRACING_ENABLED) {
        var line = tok.m_line + "." + tok.m_column + ":" + "erroneous token '" + tok.m_lexem + "'.";
        this.trace(line)
      }
    }
  ...
  Parser.prototype.findBestInterpretation = function() {
    if(this.m_erroneous_pool.length !== 0) {
      return this.m_erroneous_pool[this.m_erroneous_pool.length - 1]
    }
  ...
  Parser.prototype.recoverOnErrClearingStackAndCurrentPathAdjustingScannerPosition = function() {
    ...
    if(this.m_current.m_BP != null) {
      if(this.m_resync) {
        var level = 0;
        while(this.m_current.getPathLATokenNum() != Constants.NUM_EOF && (this.m_current.getPathLATokenNum() == this.m_scanner.getActualNUMID() || (level = this.determineStackLevelsToPopViaByFollowSet(this.m_current.m_BP.getStackframe(), this.m_current.getPathLATokenNum())) === 0)) {
          this.nextToken()
        }
