...
sap.ui.define([	"jquery.sap.global", "sap/ui/comp/smartfield/BindingUtil", "sap/ui/model/ParseException" ], function(jQuery, BindingUtil, ParseException) { // EXC_JSHINT_002 //EXC_JSHINT_034 //EXC_JSHINT_037
	...
	FieldControl.prototype.getBindableAttributes = function(bNoMandatory) {
		...
		if (oConfig && (oConfig.configdata && !oConfig.configdata.isUOM || !oConfig.configdata)) { //EXC_JSLINT_081
			return [
				"editable"
			];
		}
	...
	FieldControl.prototype._getEditable = function(oMetaData, oBindingInfo, sProperty) {
		...
		return {
			...
			formatter: function(v, p1, p2, p3) { // EXC_JSHINT_002
				var aArgs = [];

				// check the modeled static values.
				// TODO: enable creatable as soon as we have the necessary additions.
/*				
				var oBindingContext = that._oParent.getBindingContext();
				if (!oBindingContext) {
					return false; // technical exit due to BindingContext creation
				}

				if (oBindingContext.getObject) {
...
*/
				// no creatable exists -> instance is already created -> check for updatable
				if (oMetaData.property && oMetaData.property.property && !that._getUpdatableStatic(oMetaData)) {
					return false;
				}
	 ...
	 * @param {object} oMetaData.property the OData property definition
	 * @param {string} oMetaData.path the binding path
	 * @returns {boolean} <code>uomEditState</code>, if a formatter for the property <code>uomEditState</code> of a control can be returned, <code>false</code> otherwise 
	 *  @public
	 */
