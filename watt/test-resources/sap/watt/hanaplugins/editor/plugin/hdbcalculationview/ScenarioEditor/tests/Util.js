define(['sap/hana/ide/editor/plugin/analytics/base/common', 'sap/hana/ide/editor/plugin/analytics/base/modelbase'], function(common, modelbase) {
    "use strict";
    ...
    function moduleUI5(moduleName, parentId) {
        ...
        function testDone(details) {
            var elem = $('#' + parentId);
            var oldControls = controls.slice(0);

            // wait for async rendering (applyChanges) to be finished before destroying controls
            setTimeout(function() {
                while (oldControls.length > 0) {
                    var control = oldControls.pop();
                    control.destroy();
                }
                if (elem.size() !== 0) {
                    elem.empty();
