...
define([
	...
	"./proposalUtils",
	"./scriptedLogger",
	"sap/watt/lib/doctrine/doctrine"
	], function(mVisitor, typeEnv, typeInf, typeUtils, proposalUtils, scriptedLogger) {
	...
	function createProposalHelp(targetTypeName, type, prop, env) {
		var helpDescription;
		var helpTarget;
		var helpCategory;
		var helpLibrary;
		...
		var typeObj = env._allTypes[propTypeObj.name];
		if (typeObj) {
			// !description for class & function
			if (typeObj.$$category) {
				helpLibrary = typeObj.$$library;
				helpCategory = typeObj.$$category;
				var targetName = targetTypeName;
				var pos = targetName.indexOf("_obj");
				if (pos > 0) {
					targetName = targetName.substring(0, pos);
				}
				if (targetName != "Global") {
					helpTarget = targetName + "." + prop;
				} else {
					helpTarget = prop;
				}
				if (typeObj.$$description) {
					helpDescription = typeObj.$$description;
				}
			} else {
				typeObj = env._allTypes[targetTypeName + '.' + prop];
				if (!typeObj) {
					typeObj = env._allTypes[prop];
				}
				if (typeObj && typeObj.$$category) {
					...
					helpTarget = targetTypeName + '.' + prop;
				}
			}

			if (helpCategory && !helpDescription) {
				var caption = helpCategory.charAt(0).toUpperCase() + helpCategory.slice(1);
				helpDescription = caption + " " + helpTarget;
			}
		}

		return {
			category: helpCategory || "object",
	...
	function createInferredProposals(targetTypeName, env, completionKind, caseSensitive, prefix, replaceStart, proposals, relevance) {
		var prop, propTypeObj, propName, res, type = env.lookupQualifiedType(targetTypeName),
			proto = type.$$proto;
		if (!relevance) {
			relevance = 100;
		}
		...
		if (proto) {
			createInferredProposals(proto.typeObj.name, env, completionKind, caseSensitive, prefix, replaceStart, proposals, relevance - 10);
		}
		...
		for (prop in type) {
			if (type.hasOwnProperty(prop) || (!protoIsObject && realProto.hasOwnProperty(prop))) {
				...
				if (proposalUtils.looselyMatches(prefix, propName, caseSensitive)) {
					propTypeObj = type[prop].typeObj;

					var helpObj = createProposalHelp(targetTypeName, type, prop, env);
					...
					if (propTypeObj.type === 'FunctionType') {
						...
						proposals["$" + propName] = {
							...
							library: helpObj.library,
							category: helpObj.category,
							description: funcDesc,
							helpDescription: helpObj.description,
							helpTarget: helpObj.target,
	...
	function createNoninferredProposals(environment, prefix, replaceStart, proposals) {
		...
		function isInterestingProperty(type, prop) {
			return type.hasOwnProperty(prop) && prop.indexOf(prefix) === 0 && !proposals['$' + prop] && prop !== '$$proto' && prop !==
				'$$isBuiltin' &&
			...
			case 'UnionType':
				if (typeObj.elements) {
					typeObj.elements.forEach(function(elt) {
						visitTypeStructure(elt, operation);
					});
				}
				return;
	...
	function filterTypes(environment, kind, moduleTypeObj, provided) {
		var moduleTypeName = doctrine.type.stringify(moduleTypeObj, {
			compact: true
		});
		var allTypes = environment.getAllTypes();
			...
			for (var defName in type) {
				if (type.hasOwnProperty(defName)) {
					...
					if (defName === '$$fntype') {
						...
						def.typeSig = doctrine.type.stringify(typeObj, {
							compact: true
						});
						delete def._typeObj;
					}
		...
		if (typeof provided === 'object') {
			for (var defName in provided) {
				if (provided.hasOwnProperty(defName)) {
					...
					if (defName === '$$fntype') {
						inlineFunctionTypes(def, allTypes, fnTypes);
					} else if (defName === 'jQuery' || defName === 'sap') {
						delete provided[defName];
					} else {
						if (def.typeObj) {
							var typeObj = def.typeObj;
							if (fnTypeRef(typeObj, allTypes)) {
	...
	function filterAndSortProposals(proposalsObj) {
		...
		for (var prop in proposalsObj) {
			if (proposalsObj.hasOwnProperty(prop)) {
				if (proposalsObj[prop]) {
					var proposal = proposalsObj[prop].proposal;
					if (proposal && proposal.indexOf('$$') == 0) {
						continue; // ignore internal proposals
					}
				}
	...
	EsprimaJavaScriptContentAssistProvider.prototype = {
		/**
		 * Implements the WATT auto complete API
		 */
		computeProposals: function(buffer, context) {
			var ast = mVisitor.parse(buffer);
			return this._computeProposalsFromAST(ast, buffer, context);
		},

		computeProplsalsFromAST: function(ast, buffer, context) {
			return this._computeProposalsFromAST(ast, buffer, context);
		},
		
		computeSummaryFromMeta: function(sTargetNS,sMetaSymbol) {
			return this._computeSummaryFromMeta(sTargetNS,sMetaSymbol);//   Q([]);
		},

		calculateFunctionProposal: calculateFunctionProposal,
		...
		_computeProposalsFromAST: function(ast, buffer, context) {
			...
			try {
				...
				if (completionKind) {
					var environmentPromise = typeEnv.createEnvironment({
						buffer: buffer,
						uid: "local",
						offset: offset,
					...
					var result = environmentPromise.then(function(environment) {
						...
						try {
							...
							var proposalsObj = {};
							createInferredProposals(target, environment, completionKind.kind, context.caseSensitive, context.prefix, offset - context.prefix.length,
								proposalsObj);
		...
		_computeSummaryFromMeta: function(sTargetNS, sMetaSymbol) {
			var oProvided = {}, oRefTypes = {};
			var environmentPromise = typeEnv.createEnvironment({
				buffer: sMetaSymbol,
				uid: "local",
				offset: 0
			...
			var result = environmentPromise.then(function(environment) {
				try {
					var oTarget = environment.lookupQualifiedType(sTargetNS);
					if ( !oTarget || !oTarget.hasOwnProperty(sMetaSymbol) ) {
						return Q({provided: {}, types: {}});
					}	
					var oMetaType = oTarget[sMetaSymbol];
					var oMetaDef = environment.findType(oMetaType.typeObj);
					/*
					** Instantiate able object case: return Constructor function producing corresponding object type
					*/
					if ( oMetaDef.$$newtype) {
						var oMetaNewDef = environment.findType(oMetaDef.$$newtype.typeObj);	
						oRefTypes[oMetaDef.$$newtype.typeObj.name] = {};
						oRefTypes[oMetaDef.$$newtype.typeObj.name].$$category = oMetaNewDef.$$category;
						oRefTypes[oMetaDef.$$newtype.typeObj.name].$$library = oMetaNewDef.$$library;
						for ( var refProp in oMetaNewDef) {
							if ( oMetaNewDef.hasOwnProperty(refProp) && refProp.charAt(0) !== '$' ) {
								var oMetaDefProp = oMetaNewDef[refProp];
								/* Add types for the 1 level type for the POC purposes */
								var _pocNewMetaDefTypeObjName = oMetaDef.$$newtype.typeObj.name + "#" + refProp;
								oRefTypes[_pocNewMetaDefTypeObjName] = environment.findType(oMetaDefProp.typeObj);
								var _pocMetaDefPropTypeObj = { type: oMetaDefProp.typeObj.type, name: _pocNewMetaDefTypeObjName };
								oMetaDefProp = new typeUtils.Definition(_pocMetaDefPropTypeObj, oMetaDefProp.range, oMetaDefProp.path);
								/* */
								oRefTypes[oMetaDef.$$newtype.typeObj.name][refProp] = oMetaDefProp;
							}
						}
						if (oMetaDef.$$fntype) {
							var providedType = doctrine.type.stringify(oMetaDef.$$fntype, {
							compact: true
							});
							oProvided = providedType;
						}
					} else {
						oProvided = oMetaDef;
						for ( refProp in oMetaDef) {
							if ( oMetaDef.hasOwnProperty(refProp) && refProp.charAt(0) !== '$' ) {
								_pocMetaDefPropTypeObj = environment.findType(oMetaDef[refProp].typeObj);
								if ( ( _pocMetaDefPropTypeObj.$$isBuiltin && !_pocMetaDefPropTypeObj.$$library )
														|| oRefTypes.hasOwnProperty(oMetaDef[refProp].typeObj.name)) {
									continue;
								}
								oRefTypes[oMetaDef[refProp].typeObj.name] = _pocMetaDefPropTypeObj;
							}
						}
					}
...
*/					// TODO: build summary
		...
		_internalFindDefinition: function(root, buffer, offset, findName) {
			var toLookFor;
			if (!root) {
				// assume a bad parse
				return Q();
			...
			// TODO: Integrate into shouldVisist 
			try {
			...
			var completionKind = shouldVisit(root, offset, toLookFor.name, buffer);
			...
			return environmentPromise.then(function(environment) {
				environment.defer = completionKind.toDefer;
				if (environment.defer) {
					// remove these comments from consideration until we are inferring the deferred
					environment.deferredComments = extractDocComments(environment.comments, environment.defer.range);
				...
				if (maybeType) {
					...
					var hover = typeUtils.styleAsProperty(lookupName, findName) + " : " + typeUtils.createReadableType(maybeType.typeObj, environment,
						true, 0, findName);
					maybeType.hoverText = hover;
					maybeType.hoverName = lookupName;
					maybeType.hoverRange = toLookFor.range;
					return Q(maybeType);
				} else {
					return Q({
						"hoverName": lookupName,
						"hoverRange": toLookFor.range
					});
				}
			});
		},

		_internalFindDefinitionOLD: function(root, buffer, offset, findName) {
			var toLookFor;
			if (!root) {
				// assume a bad parse
				return Q();
		 ...
		 */
		computeHover: function(ast, buffer, offset) {
			return this._internalFindDefinition(ast, buffer, offset, true);
		},

		findDefinition: function(ast, buffer, offset) {
			return this._internalFindDefinition(ast, buffer, offset, false);
		},
		...
		computeSummary: function(buffer, fileName) {
			...
			return environmentPromise.then(function(environment) {
				...
				} else if (environment.SapUIAmd && environment.SapUIAmd.result) {
					// SAPUI5 module defined via sap.ui.define
					modTypeObj = environment.SapUIAmd.result;
					providedType = environment.findType(modTypeObj);
					kind = "AMD";
				
				} else if (environment.SapUIAmd) {
					var args = environment.SapUIAmd.expression["arguments"];
					if (args && args.length > 0) {
						modTypeObj = typeUtils.extractReturnType(environment.getFnType(args[args.length - 1]));
					} else {
						modTypeObj = typeUtils.OBJECT_TYPE;
				...
				} else if (environment.SapUI5) {
					// SAPUI5 module defined via jQuery.sap.declare
					modTypeObj = environment.SapUI5.typeObj;
					providedType = environment.SapUI5.moduleType;
					sapUI5ModuleId = environment.SapUI5.moduleId;
				...
				if (!typeUtils.isFunctionOrConstructor(modTypeObj) && !environment.findType(modTypeObj).$$isBuiltin) {
				...
				if (!environment.SapUI5 && !environment.SapUIAmd) {
				...
				if (typeUtils.isFunctionOrConstructor(modTypeObj) ||
					...
					providedType = doctrine.type.stringify(modTypeObj, {
						compact: true
					});
				} else if (providedType.$$fntype) {
					providedType = doctrine.type.stringify(providedType.$$fntype, {
						compact: true
					});
				}
