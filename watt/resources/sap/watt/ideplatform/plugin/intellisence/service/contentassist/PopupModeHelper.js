	...
	var showPopup = function () {
		...
		if (_oPopupMode.parent.aProposals.length > 0) {
			...
			for (k = 0; k < _oPopupMode.parent.aProposals.length; k++) {
				var iconPath=_oPopupMode._getIconPath(_oPopupMode.parent.aProposals[k].category);
				
				var helpDes = _oPopupMode.parent.aProposals[k].helpDescription;
				var bStrike = false;
				...
				if (iconPath!=="") {	
				    ...
				    if(bStrike){
    					html += '<li  selected_id="' + k + '"' + ' class="' + sItemClass + '-item"><img src="' + iconPath + '" /><span style="text-decoration:line-through;" class="intellisense_description ' + sItemClass + '">' +
    						jQuery.sap.encodeHTML(_oPopupMode.parent.aProposals[k].description) + '</span></li>';
				    }else{
					...
					//sItemClass = _oPopupMode.parent.aProposals[k].unselectable ? "unselectable" : "selectable";
					if(bStrike){
					   html += '<li class="proposalitem '+sItemClass+'-item" selected_id="' + k + '"><span style="text-decoration:line-through;" class="' + sItemClass + '">' + jQuery.sap.encodeHTML(_oPopupMode.parent.aProposals[k].description) + '</span></li>';
					}else{
					   html += '<li class="proposalitem '+sItemClass+'-item" selected_id="' + k + '"><span class="' + sItemClass + '">' + jQuery.sap.encodeHTML(_oPopupMode.parent.aProposals[k].description) + '</span></li>';
					}
				}
			}
		}
			...
			if (category != undefined) {
				for (var i = 0; i < this.parent._aCategoryIcon.length; i++) {
					if (this.parent._aCategoryIcon[i].type == category) {
						if (!this.parent._aCategoryIcon[i].isExternal) {
							...
							return require.toUrl(this.parent._aCategoryIcon[i].path);
						}						
					}
				}
			}
