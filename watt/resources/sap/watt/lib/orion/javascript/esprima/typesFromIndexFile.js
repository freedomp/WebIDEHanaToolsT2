...
define([
	"./typeUtils",
	"./indexFiles/ecma5Index",
	"./indexFiles/browserIndex",
	"sap/watt/lib/doctrine/doctrine"
	], function (typeUtils, ecma5, browser) {
	...
	function ternSig2ClosureSigInternal(ternSig, returnTypeName, typeInfo) {
		...
		function parseArgs(str) {
			...
			while (leftToParse.slice(0,1) !== ")" && leftToParse.indexOf(")") >= 0) {
				...
				if (leftToParse.slice(0, 2) === "fn") {
					...
					if (leftToParse.indexOf(",") < leftToParse.indexOf(")")) {
	 ...
	 */
	function definitionForType(typeInfo, type, name, libName) {
	
		/**
		 * parse the properties from type, with special handling
		 * of the "!type" property
		 */
		function parseObjType(type, name, libName) {
			var propInfo = {}, def;
			if (libName) {
				propInfo.$$library = libName;
			}
			for (var p in type) {
				if (hop(type, p)) {
					if (p === "!type") {
						...
						propInfo.$$description = type[p];
					} else if (p === "!category") {
						propInfo.$$category = type[p];
					} else if (p === "!url" || p === "!stdProto" || p === "!effects" || p === "!doc") {
						// do nothing for now
					} else if (p[0] === '!') {
						throw "didn't handle special property " + p;
					} else if (p === "prototype") {
						propInfo[p] = definitionForType(typeInfo, type[p]);// keep prototype as well
						...
						propInfo[p] = definitionForType(typeInfo, type[p], undefined, libName);
					}
				}
			}
		...
		if (typeof type === "string") {
			...
			// to support the full module name with namespace
			/*
			if (type.indexOf(".") !== -1) {
			...
			var parsed = parseObjType(type, name, libName);
			...
			if (newTypeName && newTypeName !== "Global" && !emptyTypeName(type["!type"])) {
				if (newTypeName === "Object") {
					...
					if (typeInfo[newTypeName]) {
						...
						//console.warn("type " + newTypeName + " defined twice in index file");
						
						// merge the object, instead of overwrite
						parsed.propInfo.$$isBuiltin = true;
						for (var propType in parsed.propInfo) {
							if (!typeInfo[newTypeName][propType]) {
								typeInfo[newTypeName][propType] = parsed.propInfo[propType];
							}
						}
					} else {
						// mark as built-in so not mucked up by inference
	 ...
	 */
	function addIndexInfo(json, globals, typeInfo, libName) {
		var p;
		for (p in json) {
			if (hop(json, p)) {
				if (p === "!name" || p === "!version") {
					...
					for (var n in anonTypes) {
						if (hop(anonTypes, n)) {
							...
							// information
							definitionForType(typeInfo, anonTypes[n], n, libName);
						}
					}
				} else if (typeof json[p] === "string") {
					var typeStr = json[p];
					var typeObj = { "!type": typeStr };
					globals[p] = definitionForType(typeInfo, typeObj, p, libName);
				} else {
					// new global
					var type = json[p];
					globals[p] = definitionForType(typeInfo, type, p, libName);
				}
			}
		}
	}
	
	function removeDefinitionForType(typeInfo, type, name) {	
		var newTypeName = name;
		var obj = typeInfo[newTypeName];
		if (obj){
			for (var prop in obj){
				if (prop.indexOf("$$") !== 0){
					if (obj[prop]._typeObj &&
						obj[prop]._typeObj.name && 
						obj[prop]._typeObj.name.indexOf("gen~index~") == 0) {
						removeDefinitionForType(typeInfo, type, obj[prop]._typeObj.name);
					}
				}
			}
			if (typeInfo[newTypeName]) {
				typeInfo[newTypeName] = undefined;
				delete typeInfo[newTypeName];
			}
		}	
	}
	...
	function init() {
		if (!isECMA5Loaded) {
			...
			addIndexInfo(browser, globalPrototype, typesPrototype);
			d.resolve();
	...
	function loadIndexInfo(indexInfo, libName) {
		try {
			addIndexInfo(indexInfo, globalPrototype, typesPrototype, libName);
		} catch (e){
			return false;
		}
		return true;
	...
	function addLibrary(knownTypes, libName) {
		...
		if (!globalsAndTypes) {
			...
			// var indexFile;
			// if (libName === "browser") {
				// indexFile = "esprima/indexFiles/browserIndex";
			// } else if (libName === "node") {
				// indexFile = "esprima/indexFiles/nodeIndex";
			// } else {
	...
	function reset() {
        for( var type in typesPrototype) {
            delete typesPrototype[type];
         }
	     _typeCount = 0;
         addIndexInfo(ecma5, globalPrototype, typesPrototype);
         addIndexInfo(browser, globalPrototype, typesPrototype);
	...
	return {
		...
		loadIndexInfo : loadIndexInfo,
		addIndexInfo: addIndexInfo,
		reset: reset
	};
});