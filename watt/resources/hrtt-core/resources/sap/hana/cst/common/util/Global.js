...
(function() {
	...
	var framebuster = function() {
		...
		if (self === top || fnIFrameIsCreatedByTestResourceFromSameServer()) {
			...
			top.location = validateAntiXSRSToken(self.location);
		}
	};

	function validateAntiXSRSToken(argument) {
        return argument;
    }
	...
	var fnLoadEnv = function(fnCallback, sEnvPath) {
		...
		var search = validateAntiXSRSToken(window.location.search);
		var url = search ? sEnvPath + search : sEnvPath;
	...
	sap.watt.network = {
		...
		getNetworkState : function() {
			...
			xhr.open("GET", "/?rand=" + Math.floor((1 + Math.random()) * 0x10000), false);
			try {
				xhr.send();
					...
					function(mEnv) {
						...
						if ((window.requirejs && !(window.jQuery && window.jQuery.sap))||bLeanWebIde) {
							...
							var loadMain = function() {
								...
								require(
										[ "sap/watt/core/q", "sap/hana/cst/common/util/UI5Loader", "sap/watt/core/DebugHooks" ],
										function(Q, ui5, debug) {
