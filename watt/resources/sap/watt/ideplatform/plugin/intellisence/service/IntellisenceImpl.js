define([
	"./contentassist/LinkModeHelper",
	"./contentassist/HintModeHelper",
	"./contentassist/PopupModeHelper",
	"./contentassist/TermModeHelper"
	...
	return {
		...
		executeIntellisence: function() {
			this.context.service.usagemonitoring.startPerf("editor", "get_completion_content").done();
			...
			return oReadyPromise.spread(function(oCodeCompletionService, oInnerEditorService) {
				...
				return oInnerEditorService.getReadOnly().then(function(readOnly) {
					...
					return oInnerEditorService.getContentStatus(true, true).then(function(contentStatus) {
						...
						if (oContentStatus.stringValue != null && aProposals.proposals.length === 0 && aProposals.isValue !== false) {
							...
							that.context.service.usagemonitoring.report("editor", "get_completion_content",oCodeCompletionService._sName).done();
						}
					});
				});
			});
		...
		getCalculatedLibraries: function(oDocument) {
			var that = this;
			var oContentService = this.context.service.content;
			var oCalculatedLibVersionService = this.context.service.intellisence.calculatelibraryversion;
			var oDocumentPromise = oDocument ? Q(oDocument) : oContentService.getCurrentDocument();
			return oDocumentPromise.then(function(oDocument) {
			...
			if (status) {
				if (status.stringValue != null) { // inside a quotation mark
					...
					return oEditorService.getSelection().then(function(selection) {
						return oTranslationSevice.showGetTermsUI(that.replaceByTerm, status.stringValue, false, false, true, {
