...
(function () {
  ...
  function GitGraph(options) {
    ...
    this.selectedDiv.className = "gitgraph-selected";
    //this.selectedDiv.style.position = "fixed";
    this.selectedDiv.style.display = "none";

    // Add selected div into body
...
//<<<<<<<<<<<<<<<<<<<<<<<<<<<    
    ...
    if (!this.template.commit.message.display && !this.template.commit.message.hidehovermessage ) {
  ...
  **/
  ...
  GitGraph.prototype.render = function () {
    ...
    if (this.template.commit.spacingY > 0) {
      this.context.translate(0, unscaledResolution.y - this.marginY * 2);
      this.offsetY = unscaledResolution.y - this.marginY * 2;
    }
    if (this.template.commit.spacingX > 0) {
      this.context.translate(unscaledResolution.x - this.marginX * 2, 0);
      this.offsetX = unscaledResolution.x - this.marginX * 2;
    }
  ...
  GitGraph.prototype.hover = function (event) {
    ...
    GitGraph.prototype.click = function (event) {
        ...
        function emitMousedownEvent () {
            var mousedownEventOptions = {
                index: commit.index
            };
            _emitEvent( self.canvas, "commit:mousedown", mousedownEventOptions );
        }
...
//<<<<<<<<<<<<<<Fix Delete issue  
        this.parent.columnsArr[this.column] = false;
//<<<<<<<<<<<<<Fix delete issue        
        this.isfinish = true;
  };
...
//<<<<<<<<<<<<<<<<<<<<<Fix delete issue    
 // for (var i = 0, branch; !! (branch = this.parent.branchs[i]); i++) {
    ...
    var bNodeChaged = false;
    for (var i = 0; i < this.parent.columnsArr.length; i++) {
    	if(!this.parent.columnsArr[i]){
    	    this.parent.columnsArr[i] = bNodeChaged = true;
    	    this.column = i;
    	    break;
    	} 
    }
    ...
    if(!bNodeChaged){
        this.column = this.parent.columnsArr.length;
        this.parent.columnsArr.push( true );
    }    
//<<<<<<<<<<<<<<<<<<<<End Fix delete issue    
  ...
  Commit.prototype.render = function () {
    ...
    if (this.messageDisplay) {
      ...
      if (this.template.commit.message.messageFormat && this.messageArgs.length > 0){    	  
    	  message = this.template.commit.message.messageFormat.format(this.messageArgs);
      }
   ...
   * @param {Boolean} [options.selectevent] - Active select mouse event or not
  ...
  function Template(options) {
    ...
    this.selectevent =  (typeof options.selectevent === "boolean") ? options.selectevent : false;
    ...
    this.commit.message.hidehovermessage = (typeof options.commit.message.hidehovermessage === "boolean") ? options.commit.message.hidehovermessage : false;
    
    this.commit.message.messageFormat = options.commit.message.messageFormat;
