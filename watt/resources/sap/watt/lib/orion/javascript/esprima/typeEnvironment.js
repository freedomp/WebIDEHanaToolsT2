		...
		var result = promise.then(function(knownTypes) {
			return {
				...
				addOrSetVariable: function(name, target, typeObj, range, docRange) {
					...
					var current = this._allTypes[targetTypeName],
						found = false;
				...
				lookupQualifiedType: function(name, includeDefinition) {
					...
					if (name) {
						...
						targetType = this._allTypes[scopeNames[0]];
					...
					if (!targetType) {
						// avoid to return 'undefined'
						targetType = this._allTypes["Object"];
					}
					return targetType;
				},
						...
						if (summary.types.hasOwnProperty(typeName)) {
							...
							if (!type || type.$$isBuiltin) {
								...
								for (var typeProp in existingType) {
									if ( typeProp === "$$category" ) {
										type.$$category = existingType.$$category;
									} else if ( typeProp === "$$library" ) {
										type.$$library = existingType.$$library;
									} else
									if (!type[typeProp]) {
										type[typeProp] = typeUtils.Definition.revive(existingType[typeProp]);
									}
								}
							} else if (type && type.$$isNS && existingType.$$isNS) {
								// console.log(".............merge NS type " + typeName);
								for (var typeProp in existingType) {
									if (!type[typeProp]) {
										type[typeProp] = typeUtils.Definition.revive(existingType[typeProp]);
									}
				...
				updateObjLitFunctionType: function(objLitNode, propName, funcExpNode) {
					var objLitTypeName = objLitNode.extras.inferredTypeObj.name,
						funcTypeName = funcExpNode.extras.inferredTypeObj.name;
					if (objLitTypeName && funcTypeName) {
						...
						// replace function proto when the function is from ui5 call expression
						if (this.ui5CallExpression && this.ui5CallExpression.callee) {
							var objProperties = this.ui5CallExpression.properties;
							for (var i in objProperties) {
								var objProperty = objProperties[i];
								// only the same func name & func range will be replaced
								if (objProperty.key && objProperty.key.name === propName &&
									objProperty.value && objProperty.value.range &&
									objProperty.value.range[0] === funcExpNode.range[0] &&
									objProperty.value.range[1] === funcExpNode.range[1]) {
									var funcExpProtoType = this._allTypes[this._allTypes[funcTypeName].$$prototype.typeObj.name];
									// e.g. replace from 'Object' to 'sap.ui.core.mvc.Controller' to support 'this'
									funcExpProtoType.$$proto.typeObj.name = this.ui5CallExpression.callee;
									break;
								}
							}
						}
