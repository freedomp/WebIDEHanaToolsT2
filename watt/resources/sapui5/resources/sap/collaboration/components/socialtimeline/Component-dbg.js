...
jQuery.sap.require("sap.collaboration.components.controls.AddPostPopover");
	...
	/** @lends sap.collaboration.components.socialtimeline.Component */
	{
		...
		*/
		...
		setBusinessObjectMap: function(oBusinessObjectMap){
			...
			var oTimelineTermsUtility = this._oInputValidator.createTermsUtilityForBackend(this._oBusinessObjectMap.serviceModel);
			var oServiceDataHandler = new sap.collaboration.components.socialtimeline.datahandlers.ServiceDataHandler(this._oBusinessObjectMap.serviceModel, oTimelineTermsUtility);
		...
		_createAddPostButton: function(){
			...
			if(this._oAddPostPopover === undefined){
				this._oAddPostPopover =  new sap.collaboration.components.controls.AddPostPopover(this.getId() + "_addPostPopover" , {
					notifyAllEnabled: false,
					postPress: this._onAddPost.bind(this),
					suggest: this._onSuggest.bind(this),
					afterSuggestionClose: function(){
						if(that.gettingSuggestions){
							that.gettingSuggestions.request.abort();
						}
					}
				});
			}
			var oAddPostButton = new sap.m.Button(this.getId() + "_addPostButton" , {
				icon: "sap-icon://add",
				type: sap.m.ButtonType.Transparent,
				enabled: false,
				press: function(){
					that._oAddPostPopover.openBy(this);
				}
			});
		...
		_refreshTimelineModel: function(){
			...
			this._oGettingTimelineData = 
				this._oTimelineDataHandler.getTimelineData(this._oFilter).then(
					function(oTLData){
						...
						that._hideReplyFromJamBarInTLItems(); // hide the replies from the TL items that are TL entries
		...
		_addTimelineItemToTimeline: function(oTLItemData){
			...
			this._oTimeline.addContent(oTimelineItem);
				...
				this._oGettingTimelineData = 
					...
					.then(
						function(oTLData){
							...
							that._hideReplyFromJamBarInTLItems();
						},
						function(oError){
							 that._oCommonUtil.displayError();
					});
		...
		_onAddPost: function(oControlEvent){
			...
			if(sContent && sContent.trim() !== ""){
				this._oTimeline.setBusyIndicatorDelay(0).setBusy(true);
				this._oTimelineDataHandler.getExternalObjectFromInternalObject()	// 1- get the external object
				.then(function(oExternalObject){
					return that._oJamDataHandler.addPostToExternalObject(sContent, oExternalObject);	// 2- add a post for the external object 
				}) 
		...
		_onSuggest: function(oEventData){
			...
			var oPopover;
			var oSource = oEventData.getSource();
			oSource.getMetadata().getName() === "sap.collaboration.components.controls.AddPostPopover" ? 
					oPopover = this._oAddPostPopover : oPopover = oSource.getParent().getCustomReply();
				...
				this.gettingSuggestions.promise.done(function(oData, response){
					...
					if(aJamResults.length === 0){ // if nothing is returns from jam then close the suggestion popover
						...
						for(var i = 0; i < aJamResults.length; i++){
							aSuggestions.push({ fullName: aJamResults[i].FullName, 
												email: aJamResults[i].Email, 
												userImage: that._defaultAttributes.collaborationHostServiceUrl + "/Members('" + aJamResults[i].Id  +"')/ThumbnailImage/$value" });
						}
