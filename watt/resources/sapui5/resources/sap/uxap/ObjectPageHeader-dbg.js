...
sap.ui.define(['sap/ui/core/Control', 'sap/ui/core/IconPool', 'sap/uxap/BreadCrumbs', 'sap/ui/core/ResizeHandler' , './library'],
	function (Control, IconPool, BreadCrumbs, ResizeHandler, library) {
		"use strict";
	...
	ObjectPageHeader._internalAggregationFactory = {
		"_objectImage": function (oParent){
			...
			if (sObjectImageURI.indexOf("sap-icon://") == 0) {
				oObjectImage = new sap.ui.core.Icon();
			} else {
				oObjectImage = new sap.m.Image({
		...
		"_overflowActionSheet": function () {
			return new sap.m.ActionSheet({placement: sap.m.PlacementType.Bottom});
		},
		...
		"_breadCrumbs": function (oParent) {
			return new BreadCrumbs({
				...
				currentLocation: new sap.m.Text({
		...
		if (aActions.length > 1 || this._hasOneButtonShowText(aActions)) {
			...
			jQuery.each(aActions, jQuery.proxy(function (iIndex, oAction) {
				...
				if (oAction instanceof sap.m.Button && oAction.getVisible()) {
					if (oAction instanceof sap.m.Button && (oAction.getType() === "Default" || oAction.getType() === "Unstyled")) {
	...
	ObjectPageHeader.prototype.onAfterRendering = function () {
		...
		if (this.getShowPlaceholder()) {
			jQuery(".sapUxAPObjectPageHeaderObjectImage").error(function () {
				jQuery(this).hide();
	...
	ObjectPageHeader.prototype._adaptActions = function (iAvailableSpaceForActions) {
		var bMobileScenario = jQuery("html").hasClass("sapUiMedia-Std-Phone") || sap.ui.Device.system.phone,
	...
	ObjectPageHeader.prototype._getActionsWidth = function () {
		...
		this.getActions().forEach(function (oAction) {
			if (oAction instanceof sap.m.Button) {
				oAction.$().show();
