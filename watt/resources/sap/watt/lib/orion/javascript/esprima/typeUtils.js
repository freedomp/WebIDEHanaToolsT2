...
define([
	...
	], function(proposalUtils, scriptedLogger) {
	...
	function ensureTypeObject(signature) {
		...
		try {
			...
			//console.warn("doctrine failure to parse: " + signature);
			return {};
		}
	}
	...
	Object.defineProperty(Definition.prototype, "typeObj", {
		set: function typeObj(val) {
			var maybeObj = val;
		...
		convertJsDocType : function(jsdocType, env, doCombine, depth) {
			...
			switch (jsdocType.type) {
				...
				case 'UnionType':
					return {
						type: jsdocType.type,
						elements: ( jsdocType.elements && Array.isArray(jsdocType.elements) ) ? jsdocType.elements.map(function(elt) {
				...
				case 'FunctionType':
					var fnType = {
						type: jsdocType.type,
						params: ( jsdocType.params && Array.isArray(jsdocType.params) ) ? jsdocType.params.map(function(elt) {
							return self.convertJsDocType(elt, env, doCombine, depth);
						}) : []
					};
					if (jsdocType.result) {
...
//					}
		...
		createReadableType : function(typeObj, env, useFunctionSig, depth, useHtml) {
			if (useFunctionSig) {
				...
				res = this.limitReadableLength(res, 80);
				return res;
			} else {
		...
		limitReadableLength: function (res, length) {
			if (res && res.length > length && 
				res.indexOf('{') == 0 && res.lastIndexOf('}') == res.length-1) {
				var newRes = "";
				var pos = res.indexOf(',');
				while (pos > 0) {
					if (newRes.length + pos > length) {
						break;
					}
					
					newRes += res.substring(0, pos + 1);
