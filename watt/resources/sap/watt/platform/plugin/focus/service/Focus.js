	...
	return {
		...
		_isDialog : function(oService) {
			...
			return this._getFocusElement(oService).then(function(oFocusElem) {
				...
				if(!oFocusElem || oFocusElem.length == 0) {
					bIsDialog = true;
				}
		...
		detachFocus : function(oService) {
			var that = this;
			this._detachedService = oService;
			...
			return this._getFocusElement(oService).then(function(aFocusElement) {
				var aPromises = [];
				that._iterateElements(aFocusElement, function(oFocusElem) {
					if (oFocusElem) {
						if(oFocusElem instanceof HTMLIFrameElement) {
						...
						aPromises.push(that._removeFocusElement(oService).then(function() {
							oFocusElem.removeEventListener("focus", jQuery.proxy(that._focusHandler, that), true);
							that._detachedService = null;
							return Q();
						}));
					}
				});
		...
		_focusHandler : function(evt) {
			...
			var that = this;
			if (evt.type === "focus") {
				var oHandlerTarget = evt.currentTarget.frameElement ? evt.currentTarget.frameElement : evt.currentTarget;
				var oTarget = evt.target;
				var sHandlerFocusIndex = oHandlerTarget.getAttribute("focusindex");
				var sTargetFocusIndex = oTarget.getAttribute ? oTarget.getAttribute("focusindex") : -1;
				
				if(this._aFocusElements["index" + sTargetFocusIndex]) {
					oService = this._aFocusElements["index" + sTargetFocusIndex];
				}else {
					oService = this._aFocusElements["index" + sHandlerFocusIndex];
				}
			}
			//check if it is the same service or if the service is in the process of being detached right now
			if (this._currentFocusElement === oService || this._detachedService === oService) {
				return;
			}
